...
describe('CrawlTasksPage manual-action helper health', () => {
  it('guides helper recovery without automatically opening a browser or resuming', async () => {
    ...
    expect(await screen.findByText('Host helper is offline')).toBeInTheDocument();
    ...
    expect(screen.getByText(/does not reuse the open browser/i)).toBeInTheDocument();

    const advanced = screen.getByText('Advanced troubleshooting').closest('details');
    ...
    expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/open-browser'))).toBe(false);
    expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/resume'))).toBe(false);

    const user = userEvent.setup();
    await user.click(screen.getByTestId('crawl-task-open-browser'));
    await act(async () => {});
    expect(screen.getByText('Browser connected — site access is not verified')).toBeInTheDocument();
    expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/resume'))).toBe(false);

    await user.click(screen.getByTestId('crawl-task-resume-open-browser'));
    await act(async () => {});
    const resumeRequest = apiFetchJson.mock.calls.find(([url]) => `${url}`.includes('/resume'));
    expect(resumeRequest?.[1]?.body).toBe(JSON.stringify({ strategy: 'reuse_open_browser' }));
  });
  ...
  it('keeps a simpler fresh-profile path for manual actions without browser reuse', async () => {
    ...
    expect(await screen.findByText('Operator review required')).toBeInTheDocument();
    expect(screen.getByText('Review the source page before resuming.')).toBeInTheDocument();
    expect(screen.getByTestId('crawl-task-resume-fresh')).toBeEnabled();
    expect(screen.queryByTestId('crawl-task-copy-helper-command')).not.toBeInTheDocument();
    expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/health'))).toBe(false);
  });
  ...
  it('confirms profile cleanup and crawl cancellation before sending either request', async () => {
    ...
    apiFetchJson.mockImplementation(async (url) => {
      ...
      if (`${url}`.includes('/close-profile-windows') || `${url}`.includes('/cancel')) {
        return { status: 'ok' };
      }
      return { items: [manualTask], total: 1, page: 1, page_size: 10 };
    });
    ...
    render(<CrawlTasksPage />);
    expect(await screen.findByText('Browser connected — site access is not verified')).toBeInTheDocument();
    ...
    expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/close-profile-windows'))).toBe(false);
    await user.click(closeButton);
    await waitFor(() => {
      expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/close-profile-windows'))).toBe(true);
    });

    const cancelButton = screen.getByTestId('crawl-task-cancel');
    await user.click(cancelButton);
    expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/cancel'))).toBe(false);
    await user.click(cancelButton);
    await waitFor(() => {
      expect(apiFetchJson.mock.calls.some(([url]) => `${url}`.includes('/cancel'))).toBe(true);
    });

    expect(confirmSpy).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining('Close the dedicated OfferToday browser profile windows'),
    );
    expect(confirmSpy).toHaveBeenNthCalledWith(
      3,
