...
from app.api import internal_recommendations as recommendations_api
...
@pytest.mark.asyncio
async def test_internal_similar_jobs_endpoint_uses_recommendation_service(monkeypatch):
    ...
    try:
        response = await client.get(f"/api/v1/internal/jobs/{RECOMMENDATIONS_RESPONSE['source_job_id']}/similar?limit=3")
    finally:
        await client.aclose()
...
@pytest.mark.asyncio
async def test_internal_recommendations_endpoint_returns_404_for_unknown_source_job(monkeypatch):
    client = _build_test_client()
    ...
    try:
        response = await client.get(f"/api/v1/internal/recommendations/jobs?job_id={uuid.uuid4()}")
    finally:
        await client.aclose()
